...
public class DBQuery {
	...
	public int update(String[] values) {
		...
		if (values.length != this.fieldsName.length) {
			System.out.println("\n A quantidade de campos eh diferente da quantidade de valores!");
			return (0);
		}
		...
		return (this.execute(sql));
		/*
		 * for( int i=0; i < values.length; i++){ sql += "\n\t "+ this.fieldsName[i] +
		 * " = '"+values[i].trim()+"'" + ((i == values.length-1) ? "" : ", "); } if (
		 * this.keyFieldIndex < 0 ){ return(0); } sql += "\n WHERE "+ this.fieldKey
		 * +" = '"+ values[this.keyFieldIndex] +"'";
		 */
	}
